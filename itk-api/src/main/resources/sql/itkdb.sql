...
CREATE TABLE `t_home_page` (
  ...
  `is_del` int(1) DEFAULT NULL COMMENT '删除状态 0:正常 1:删除',
  `type` int(1) DEFAULT NULL COMMENT '1：商家 2：农场',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
...
CREATE TABLE `t_shop_info` (
  ...
  `shop_code` VARCHAR(20) NOT NULL COMMENT '门店编码 B00001商家id，F000001农场id',
...
CREATE TABLE `t_sale_info` (
  ...
  `sale_type` int(1) DEFAULT NULL COMMENT '促销类型 （1：满减 2：现金券）',
...
CREATE TABLE `t_order_header` (
  ...
  `address_id` INT(11) NOT NULL COMMENT '地址信息 id',
...
CREATE TABLE `t_order_detail` (
  ...
  `item_amount` DECIMAL(20,4) NOT NULL COMMENT '单个商品金额',
  `actual_amount` DECIMAL(20,4) NOT NULL COMMENT '实际支付金额',
  `freight_id` INT(4) DEFAULT NULL COMMENT '运费 id',
  `platform_promotion_code` INT(10) COMMENT '平台券id',
  `platform_promotion_cost` DECIMAL(20,4) COMMENT '平台券使用金额',
  `shop_promotion_code` INT(10) COMMENT '商家券 id',
...
CREATE TABLE `t_purchase` (
  ...
  `order_id` VARCHAR(1024) NOT NULL COMMENT '订单 id',
  `is_del` INT(1) DEFAULT NULL COMMENT '删除状态 0:正常 1:删除',
...
CREATE TABLE `t_refund_flow` (
  ...
  `reject_reason_received` VARCHAR(200) DEFAULT NULL COMMENT '收货后驳回理由',
...
CREATE TABLE `t_refund_history` (
  ...
  `refund_flow_id` varchar(50) NOT NULL COMMENT '业务单据id',
...
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT = '退款历史记录表';
